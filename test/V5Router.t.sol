...
contract V5RouterForkTest is V5RouterForkTestBase {
  ...
  function test_swapUSDC() public {
    ...
    (, IAggregationRouter.SwapDescription memory desc, bytes memory permit, bytes memory data) = abi
      .decode(
      this.returnSlice(dataParams), (address, IAggregationRouter.SwapDescription, bytes, bytes)
    );
    ...
    uint256 nativeEndingBalance = nativeSwap(desc, permit, data, snapshotId);
