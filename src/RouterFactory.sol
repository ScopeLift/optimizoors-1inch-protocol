...
contract OneInchRouterFactory {
  ...
  constructor(IAggregationExecutor aggregationExecutor, IAggregationRouter v5AggregationRouter) {
  ...
  function computeAddress(address asset) external view returns (address) {
    return Create2.computeCreate2Address(
      ...
      abi.encode(V5_AGGREGATION_ROUTER, AGGREGATION_EXECUTOR, asset, SOURCE_RECEIVER)
    );
  }
